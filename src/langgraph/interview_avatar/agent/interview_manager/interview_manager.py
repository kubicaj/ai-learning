...
class InterviewManager(InterviewAgent):
    ...
    def _create_system_prompt(self, graph_state: GraphState) -> List[BaseMessage]:
        """
        ...
        system_prompt = (
            ...
            You are the interview manager which lead the interview with possible candidate

            # Your task
            - Interview candidate and keep structure of the interview
            - Request new question from interview technical lead. Provide the technical lead info about what topic you
            want to generate the question
            ...
            1.2. Introduce to candidate what will follow and check with candidate if it is ok for him
            
            ## 2. Career path
            2.1. Start with question about candidate carer
            2.2. Continue with technical questions related with candidate working experience
            
            ## 3. Technical part
            3.1. Ask candidate technical questions related with open position
                      
            ## 4. End of interview
            4.1. Ask candidate about ideal job and also ask him what the candidate do not want to do
            4.2. After that finish interview with candidate and do summarization of interview
    ...
    def agent_callback(self, graph_state: GraphState) -> GraphState:
        ...
        response = llm_with_tools.invoke(input=self._create_system_prompt(graph_state))
        generated_question = ""
